...
class BlastRadioController:
    ...
    def _load_config(self):
        ...
        try:
            ...
            if "region_w"    in data: self.saved_region_w    = data["region_w"]
            if "region_h"    in data: self.saved_region_h    = data["region_h"]
    ...
    def _setup_styles(self):
        s = ttk.Style()
        s.theme_use("clam")
        s.configure("TSpinbox",     fieldbackground=self.ENTRY_BG, foreground=self.TEXT,
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="BLAST RADIO CONTROLLER",
                 font=("Helvetica", 13, "bold"), bg=self.ACCENT, fg="white").pack(expand=True)
    ...
    def _panel_setup(self, p):
        ...
        tk.Entry(path_row, textvariable=self.app_path_var,
                 bg=self.ENTRY_BG, fg=self.TEXT, width=35).pack(side=tk.LEFT, padx=5)
        tk.Button(path_row, text="Browse", bg=self.ENTRY_BG, fg=self.TEXT,
    ...
    def _panel_signal(self, p):
        ...
        tk.Label(f,
                 text="Hover over the TOP-LEFT corner of the yellow bars, then click Pick Signal Region.",
        ...
        region_color = self.GREEN if self.signal_region else self.DIM
        self._region_lbl = tk.Label(top_row, text=region_text, bg=self.PANEL, fg=region_color)
        self._region_lbl.pack(side=tk.LEFT)

        self._signal_val_lbl = tk.Label(top_row, text="0.0%", bg=self.PANEL,
        ...
        row.pack(fill=tk.X)
        tk.Label(row, text="Activity Threshold (%)", bg=self.PANEL, fg=self.TEXT).pack(side=tk.LEFT)
        self._thresh_lbl = tk.Label(row, text=f"{self.saved_threshold:.1f}%",
                                    width=6, bg=self.PANEL, fg=self.ACCENT,
                                    font=("Helvetica", 10, "bold"))
        self._thresh_lbl.pack(side=tk.RIGHT)
        tk.Scale(f, from_=0.1, to=50.0, resolution=0.1, orient=tk.HORIZONTAL,
    ...
    def _panel_automation(self, p):
        ...
        ttk.Checkbutton(f, text="Auto-start broadcast when signal crosses threshold",
                        variable=self.auto_start_var).pack(anchor=tk.W, pady=(0, 5))
        ttk.Checkbutton(f, text="Auto-stop broadcast after signal drops below threshold",
    ...
    def _panel_controls(self, p):
        ...
        self._status_lbl = tk.Label(top, text="Idle", bg=self.PANEL, fg=self.DIM,
                                    font=("Helvetica", 11, "bold"))
        self._status_lbl.pack(side=tk.LEFT)

        self._timer_lbl = tk.Label(top, text="", bg=self.PANEL, fg=self.ACCENT,
                                   font=("Helvetica", 11, "bold"))
    ...
    def _get_blast_window(self):
        wins = [w for w in gw.getAllWindows()
                if w.title and self.APP_TITLE in w.title and "Controller" not in w.title]
